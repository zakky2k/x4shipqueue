...
from x4shipqueue.config import ALLOWED_ROLES, token_to_faction_code, NON_BUILDABLE_TOKENS, PHYSICAL_SHIP_SIZES
...
def is_buildable_hull(tokens: Set[str], size: str) -> bool:
    ...
    if size not in PHYSICAL_SHIP_SIZES:
        return False
    if tokens & NON_BUILDABLE_TOKENS:
        return False
...
def extract_ship_archetypes(x4_root: Path) -> Dict[str, ShipArchetype]:
    ...
    for f in ships_files:
        ...
        for ship in root.findall(".//ship"):
            ...
            race = faction  # canonical rule: race == faction family

            if ship_id not in archetypes:
                archetypes[ship_id] = ShipArchetype(
                    source=source,
                    ship_id=ship_id,
                    name_code=None,      # resolved later via wares.xml
                    faction=faction,
                    race=race,
                    size=size,
                    role=role.lower(),
