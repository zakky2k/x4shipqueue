...
def parse_macro_properties(root: ET.Element) -> Tuple[int, int]:
    ...
    if people is not None:
        try:
            crew = int(people.get("capacity", "0"))
        except Exception:
            crew = 0
    ...
    if hull is not None:
        for attr in ("max", "value", "hull"):
            if attr in hull.attrib:
                try:
                    hull_hp = safe_int(hull.get(attr))
                    break
                except Exception:
                    pass
...
def iter_ship_macros(x4_root: Path):
    """
    Canonical iterator over ship hull macros.

    Yields:
        (macro_id, macro_root, macro_file)
    """
    for macro_file in find_ship_macro_files(x4_root):
        try:
            root = ET.parse(macro_file).getroot()
        except Exception:
            continue

        macro = root.find("./macro") or root.find("./macros/macro")
        if macro is None:
            continue
        ...
        yield macro_id, macro, macro_file
