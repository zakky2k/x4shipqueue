...
def safe_float(v: str) -> float:
    """Convert to float; return 0.0 on invalid input."""
    try:
        return float(v)
...
def parse_inline_recipes_from_wares_xml(path: Path) -> List[Recipe]:
    """
    Parse inline production recipes in wares.xml.

    Recipes are appended ONCE per production block.
    """
    recipes: List[Recipe] = []
    ...
    for w in root.findall(".//ware"):
        ...
        for prod in w.findall(".//production"):
            ...
            if primary is not None:
                inputs = [
                    (inp.get("ware"), safe_float(inp.get("amount")))
                    for inp in primary.findall(".//ware")
                    if inp.get("ware") and inp.get("amount")
                ]
                if inputs:
                    recipes.append(
                        Recipe(product_ware_id=product_id, method="primary", inputs=inputs)
                    )
                continue
            ...
            if inputs:
                recipes.append(
                    Recipe(product_ware_id=product_id, method=method, inputs=inputs)
                )
...
def parse_recipes_from_modules_xml(path: Path) -> List[Recipe]:
    ...
    for prod in root.findall(".//production"):
        ...
        if primary is not None:
            ...
            if inputs:
                recipes.append(
                    Recipe(product_ware_id=product, method="primary", inputs=inputs)
                )
            continue
...
def build_recipe_map(recipes: List[Recipe]) -> Dict[str, Recipe]:
    ...
    for r in recipes:
        ...
        elif out[r.product_ware_id].method != "primary" and r.method == "primary":
            out[r.product_ware_id] = r
    return out


# =============================================================================
# Ship production (used by hulls, kept here for compatibility)
# =============================================================================

def extract_ship_production(x4_root: Path) -> dict[str, ShipProduction]:
    """
    Parse ship production costs from wares.xml.

    NOTE:
    This is used by hull extraction and kept here to avoid churn.
    """
    ...
    for wares_file in find_library_files(x4_root, "wares.xml"):
        ...
        for ware in root.findall(".//ware"):
            ...
            macro = ware.find("component")
            price = ware.find("price")
            production = ware.find("production")

            if macro is None or price is None or production is None:
                continue

            macro_id = macro.get("ref")
            if not macro_id:
                continue
