...
def derive_variant_label(macro_id: str) -> str:
    ...
    if m.endswith("_a_macro"):
        return "A"
    if m.endswith("_b_macro"):
        return "B"
    if m.endswith("_c_macro"):
        return "C"
...
# -------------------------------------------------------------------
# Name + property parsing
# -------------------------------------------------------------------

def parse_macro_identification_name(
    root: ET.Element,
    ttable: dict[tuple[int, int], str],
) -> str:
    """
    Resolve in-game hull name from macro identification.
    """
    ident = root.find(".//properties/identification")
    if ident is not None:
        raw = ident.get("name")
        resolved = resolve_display_name(raw, ttable)
        if resolved:
            return resolved
...
# -------------------------------------------------------------------
# Component resolution + slot counting
# -------------------------------------------------------------------
...
def count_slots_from_component(
    root: ET.Element,
